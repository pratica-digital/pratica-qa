import { ArrowLeft, ClipboardCheck, RefreshCw, UserRound } from 'lucide-react';
...
export function TestRunExecutionPage({
  ...
  const handleSubmit = async (result: TestResult, payload: ExecuteTestResultPayload) => {
    ...
    try {
      const updatedResult = await testRunsApi.execute(token, run.id, {
        ...payload,
    ...
    <div className="space-y-6">
      ...
      {results.length > 0 ? (
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-8 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <h2 className="mt-3 text-sm font-semibold text-zinc-950 dark:text-white">No results yet</h2>
